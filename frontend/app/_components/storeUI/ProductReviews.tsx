...
export function ReviewsSection({ productId }: ReviewsSectionProps) {
  ...
  const submitReview = useMutation({
    mutationFn: ({ rating, comment }: { rating: number; comment: string }) =>
      productService.submitReview(productId, rating, comment),
    onSuccess: () => {
  ...
  const { reviews = [], total_reviews = 0, average_rating = 0 } = reviewsData;
    ...
    <div className="mt-16">
      <div className="grid gap-4 mb-8">
        {isAuthenticated ? (
          <Button
            variant="default"
            className="border border-gray-900 py-3 rounded-none sm:w-[50%] hover:ring-1 hover:ring-black"
      ...
      {/* Average Rating */}
      {total_reviews > 0 && typeof average_rating === "number" && (
        <div className="flex items-center gap-4 mb-8">
          <div className="flex items-center">{renderStars(average_rating)}</div>
          ...
          {reviews.map((review) => (
            <div key={review.id} className="border-b pb-8">
              ...
              {review.verified_purchase && (
                <p className="mt-2 text-sm text-green-600">Verified Purchase</p>
              )}
            </div>
          ))}
        </div>
