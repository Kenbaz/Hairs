...
import { AdminProduct, ProductFilters, ProductResponse, Category, BulkOperationResponse } from "@/src/types";
...
class AdminProductService {
  ...
  async bulkDeleteProducts(productIds: number[]): Promise<BulkOperationResponse> {
    try {
        const response = await axiosInstance.post('/api/v1/admin/products/bulk_delete/', {
      ...
      console.error("Bulk delete error details:", {
        error,
        productIds,
        requestUrl: `/api/v1/admin/products/bulk_delete/`,
      });
      throw error;
    }
  ...
  async updateStock(
    ...
    const response = await axiosInstance.post(
      `/api/v1/admin/products/${id}/update_stock/`,
      {
        stock: quantity,
        notes,
      }
  ...
  async toggleFeatured(id: number): Promise<AdminProduct> {
    const response = await axiosInstance.post(
      `/api/v1/admin/products/${id}/toggle_featured/`
    );
    return response.data;
  }
